...
for fold_name, df_parity in FOLDS.items():
    tag = f"{MODE_TAG}_1j1t_emu_{fold_name}"
    print(f"\n{'='*60}")
    ...
    plt.title(
        rf"tW vs $t\bar{{t}}$ — Optuna History (1j1t, $e\mu$, {fold_name}, {CLASSIFICATION_MODE})",
        fontsize=13
    )
    ...
    if NUM_CLASSES == 2:
        ...
        plt.title(
            rf"Signal (tW) vs {CLASS_NAMES[1]} — ROC (1j1t, $e\mu$, {fold_name})", fontsize=12
        )
    ...
    model_path = f"{MODEL_PREFIX}_1j1t_{fold_name}.json"
...
for c in CLASS_LIST:
    ...
    plot_corr_matrix(
        ...
        f"plots/correlation_matrix/{safe_name}_correlation_matrix_1j1t_emu_{MODE_TAG}.png",
    )
...
print("   Models saved:")
print(f"     {MODEL_PREFIX}_1j1t_even.json")
print("       → trained & tested on even events only")
print(f"     {MODEL_PREFIX}_1j1t_odd.json")
