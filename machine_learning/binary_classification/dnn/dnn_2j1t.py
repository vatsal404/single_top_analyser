...
# ============================================
# Helper: evaluate AUC on DataLoader
# ============================================
...
def permutation_importance_dnn(
    model,
    X,
    y,
    w,
    ...
    baseline_pred = predict_proba(model, X)
    ...
    for i, feat in enumerate(feature_names):
        ...
        for _ in range(n_repeats):
            ...
            pred_perm = predict_proba(model, X_perm)
            ...
            aucs.append(baseline_auc - auc_perm)

        mean_importance = np.mean(aucs)

        importances.append(mean_importance)

        print(f"   {feat:30s} : {mean_importance:.6f}")
...
joblib.dump(scaler, "DNN_scaler_2j1t.pkl")

print("\n💾 Scaler saved as DNN_scaler_2j1t.pkl")
...
sss = StratifiedShuffleSplit(
    n_splits=1,
    test_size=(1.0 - OPTUNA_TUNE_FRACTION),
    random_state=42
)
...
print(f"   Tune train events : {len(X_tune)}")
...
joblib.dump(
    study.best_params,
    "best_params_2j1t.pkl"
)

with open("best_params_2j1t.txt", "w") as f:

    f.write("Best Optuna Hyperparameters\n")
    f.write("===========================\n")

    f.write(
        f"Best penalised score : {study.best_value:.6f}\n\n"
    )
...
plt.title(
    "tW vs $t\\bar{t}$ — Optuna Search History (2j1t, $e\\mu$)",
    fontsize=13
)
...
plt.savefig(
    "plots/roc_classifier/optuna_search_history_2j1t_emu.png",
    dpi=150
)
...
print("\n🚀 Training final DNN on full training set...")

print(
    f"   Architecture : "
    f"{best_params['n_layers']} layers × "
    f"{best_params['n_units']} units"
)
...
plt.title(
    "tW vs $t\\bar{t}$ — DNN Training Loss (2j1t, $e\\mu$)",
    fontsize=13
)
...
plt.savefig(
    "plots/roc_classifier/loss_vs_epoch_2j1t_emu.png",
    dpi=150
)
...
plt.title(
    "tW vs $t\\bar{t}$ — DNN AUC per Epoch (2j1t, $e\\mu$)",
    fontsize=13
)
...
plt.savefig(
    "plots/roc_classifier/auc_vs_epoch_2j1t_emu.png",
    dpi=150
)
...
print(
    ...
    f"{(y_pred_test > 0.5).mean():.4f}"
)

print(
    f"   Fraction with score > 0.9 : "
    f"{(y_pred_test > 0.9).mean():.4f}"
)
...
print(
    f"   Signal (y=1)     : "
    f"{y_pred_test[y_test == 1].mean():.4f}"
)

print(
    f"   Background (y=0) : "
    f"{y_pred_test[y_test == 0].mean():.4f}"
)
...
plt.title(
    "tW vs $t\\bar{t}$ — ROC Curve (2j1t, $e\\mu$)",
    fontsize=13
)
...
plt.savefig(
    "plots/roc_classifier/roc_curve_train_vs_test_2j1t_emu.png",
    dpi=150
)
...
plt.savefig(
    "plots/confusion_matrix/confusion_matrix_2j1t_emu.png",
    dpi=150
)
...
plt.hist(
    ...
    color="tomato",
    linewidth=1.5,
    label="Train — Background ($t\\bar{t}$)"
)
...
plt.title(
    "tW vs $t\\bar{t}$ — DNN Score Distribution (2j1t, $e\\mu$)",
    fontsize=13
)
...
plt.savefig(
    "plots/dnn_output/dnn_score_distribution_2j1t_emu.png",
    dpi=150
)
...
fi_df = permutation_importance_dnn(
    final_model,
    X_test,
    y_test,
    w_test,
...
fi_df.to_csv(
    "plots/feature_importance/feature_importance_2j1t_emu.csv",
    index=False
)
...
plt.xlabel(
    "AUC Degradation After Permutation",
    fontsize=13
)
...
plt.title(
    "tW vs $t\\bar{t}$ — DNN Feature Importance (2j1t, $e\\mu$)",
    fontsize=13
)
...
plt.savefig(
    "plots/feature_importance/feature_importance_2j1t_emu.png",
    dpi=150
)
...
print("\n💾 Model weights saved as DNN_2j1t.pt")

print("💾 Scaler already saved as DNN_scaler_2j1t.pkl")
