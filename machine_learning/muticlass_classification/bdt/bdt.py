...
axes[0].set_title(r"tW vs $t\bar{t}$ Multiclass — Log Loss (1j1t, $e\mu$)", fontsize=12)
...
axes[1].set_title(r"tW vs $t\bar{t}$ Multiclass — Error (1j1t, $e\mu$)", fontsize=12)
axes[1].legend(fontsize=11)
axes[1].grid()

plt.tight_layout()
plt.savefig("plots/roc_classifier/loss_error_vs_boosting_round_1j1t_emu.png", dpi=150)
plt.close()
...
# ============================================
# ROC Plot 1 — Signal (class 0) vs each bkg
#   Left panel : Signal OvR (combined AUC)
#   Right panel: Bkg1 vs Bkg2 (OvR)
# ============================================

fig, axes = plt.subplots(1, 2, figsize=(14, 6))

# --- Panel A: Signal vs Rest (train + test overlay) ---
ax = axes[0]
for split, y_bin, proba, w_s, ls in [
    ...
    ax.plot(fpr, tpr, color=CLASS_COLORS[0], linestyle=ls,
            label=f"{split} AUC = {auc:.3f}")

ax.plot([0, 1], [0, 1], "k--", linewidth=0.8, label="Random")
ax.set_xlabel("False Positive Rate (1 − Specificity)", fontsize=12)
ax.set_ylabel("True Positive Rate (Sensitivity)",      fontsize=12)
ax.set_title(r"Signal (tW) vs Rest — OvR ROC (1j1t, $e\mu$)", fontsize=12)
ax.legend(fontsize=11)
ax.grid()

# --- Panel B: Bkg1 vs Rest and Bkg2 vs Rest ---
ax = axes[1]
for c, label in [(1, "Bkg1 (Dilept)"), (2, "Bkg2 (SemiLept)")]:
    for split, y_bin, proba, w_s, ls in [
        ("Train", y_train_bin, proba_train, w_train, "--"),
        ("Test",  y_test_bin,  proba_test,  w_test,  "-"),
    ]:
        ...
        ax.plot(fpr, tpr, color=CLASS_COLORS[c], linestyle=ls,
                label=f"{label} {split} AUC={auc:.3f}")

ax.plot([0, 1], [0, 1], "k--", linewidth=0.8, label="Random")
ax.set_xlabel("False Positive Rate (1 − Specificity)", fontsize=12)
ax.set_ylabel("True Positive Rate (Sensitivity)",      fontsize=12)
ax.set_title(r"Background OvR ROC Curves (1j1t, $e\mu$)", fontsize=12)
ax.legend(fontsize=10)
ax.grid()

plt.suptitle(
    r"tW vs $t\bar{t}$ Dilept vs $t\bar{t}$ SemiLept — Multiclass OvR ROC (1j1t, $e\mu$)",
    fontsize=13, y=1.01
)
plt.tight_layout()
plt.savefig("plots/roc_classifier/roc_signal_vs_bkg_panels_1j1t_emu.png", dpi=150, bbox_inches="tight")
plt.close()
print("📈 ROC panel plot (signal vs bkg + bkg vs bkg) saved")
...
plt.title(
    r"tW vs $t\bar{t}$ Dilept vs $t\bar{t}$ SemiLept — All OvR ROC (1j1t, $e\mu$)",
    fontsize=12
)
plt.legend(fontsize=10)
plt.grid()
plt.tight_layout()
plt.savefig("plots/roc_classifier/roc_all_ovr_combined_1j1t_emu.png", dpi=150)
plt.close()
...
ax.set_title(
    r"tW vs $t\bar{t}$ Multiclass — Confusion Matrix (1j1t, $e\mu$)",
    fontsize=12
)
plt.tight_layout()
...
plt.suptitle(
    r"tW vs $t\bar{t}$ Dilept vs $t\bar{t}$ SemiLept — BDT Score Distributions (1j1t, $e\mu$)",
    fontsize=13
)
plt.tight_layout()
plt.savefig("plots/bdt_output/bdt_score_distributions_multiclass_1j1t_emu.png", dpi=150)
plt.close()
...
plt.title(
    r"tW vs $t\bar{t}$ Multiclass — Feature Importance by Gain (1j1t, $e\mu$)",
    fontsize=12
)
plt.tight_layout()
plt.savefig("plots/feature_importance/feature_importance_gain_multiclass_1j1t_emu.png", dpi=150)
plt.close()
...
model.save_model("BDT_multiclass_1j1t.json")
print("\n💾 Model saved as BDT_multiclass_1j1t.json")
print("\n✅ All done!")
