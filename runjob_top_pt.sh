...
eos_output_dir="/eos/uscms/store/user/vsinha/top_pt_weight"
...
ls -alh
# Extract tarball
if [ -f "top.tar.gz" ]; then
    echo "Extracting top.tar.gz..."
    tar -xzf top.tar.gz
else
...
python3 top_pt_reweighting.py "$input_dir" "$output_file"  \
    > "$log_file" 2>&1
exit_code=$?
